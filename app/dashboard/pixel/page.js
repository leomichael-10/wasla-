...
function StatCard({ label, value, loading }) {
  return (
    <div className="bg-white rounded-2xl border border-brand-50 shadow-sm p-5 flex flex-col gap-1">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-brand-50 shadow-sm p-5">
          ...
          <button
            ...
            className={`text-xs font-bold px-4 py-2 rounded-xl transition-all active:scale-95 ${
              copied
                ...
                : 'bg-brand-700 hover:bg-brand-800 text-white disabled:opacity-40 disabled:cursor-not-allowed'
            }`}
          >
        ...
        {loading ? (
          <div className="h-10 bg-brand-50 rounded-xl animate-pulse" />
        ) : pixel ? (
          <pre className="bg-[#FBF6EF] border border-brand-100 rounded-xl px-4 py-3 text-xs text-gray-700 font-mono overflow-x-auto whitespace-pre-wrap break-all select-all">
            {pixel.embedCode}
          </pre>
        ) : (
      ...
      <div className="bg-white rounded-2xl border border-brand-50 shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="p-5 space-y-2">
            {[1,2,3].map(i => <div key={i} className="h-5 bg-brand-50 rounded animate-pulse" />)}
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#FBF6EF] border-b border-gray-50">
            ...
            <tbody className="divide-y divide-gray-50">
              {stats.topReferrers.map((r, i) => (
                <tr key={i} className="hover:bg-[#FBF6EF] transition-colors">
                  ...
                  <td className="px-5 py-3 text-right font-black text-brand-700 tabular-nums">
                    {r.count}
                  </td>
                </tr>
              ))}
