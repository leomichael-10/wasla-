...
export default function ManageStockPage() {
  ...
  if (!product) return (
    <div className="max-w-3xl">
      ...
      <Link href="/dashboard/products" className="mt-3 inline-block text-brand-700 font-bold text-sm hover:underline">
        Back to products
      </Link>
    </div>
  )
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {product.variants.map((v, i) => {
                  const row = rows[i] ?? {}
                  return (
                    <tr key={v.id} className={v.stockQty < 5 ? 'bg-red-50' : 'hover:bg-[#FBF6EF]'}>
                      ...
                      <td className="px-4 py-3">
                        <input
                          ...
                          placeholder="0"
                          className="w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-400"
                        />
                      </td>
                    </tr>
                  )
