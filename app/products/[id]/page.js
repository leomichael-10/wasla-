    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-4 py-24 text-center">
          ...
          <Link href="/products" className="mt-4 inline-block text-teal-600 font-bold hover:underline text-sm">
            Back to products
          </Link>
        </div>
      </div>
          ...
          {product.category && (
            <>
              ...
              <Link href={`/products?category=${encodeURIComponent(product.category.name)}`}
                className="hover:text-teal-600 transition-colors">{product.category.name}</Link>
            </>
          )}
          <span>/</span>
        ...
        <div className="grid sm:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-3">
            {images.length > 0 ? (
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {images.map((url, i) => (
                      <button key={url} onClick={() => setMainImageIdx(i)}
                        className={`shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-colors ${i === mainImageIdx ? 'border-teal-400' : 'border-transparent hover:border-gray-300'}`}>
              ...
              <div className="bg-linear-to-br from-teal-50 via-teal-100 to-cyan-100 rounded-3xl aspect-square flex items-center justify-center shadow-sm">
                <span className="text-6xl font-black text-teal-200 select-none tracking-tighter">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <span className="text-xs font-black text-teal-600 uppercase tracking-widest">{product.brand}</span>
              <h1 className="text-2xl sm:text-3xl font-black text-gray-900 mt-1 leading-tight">{product.name}</h1>
              {product.category && (
                <Link href={`/products?category=${encodeURIComponent(product.category.name)}`}
                  className="inline-block mt-2 bg-teal-50 text-teal-700 text-xs font-semibold px-3 py-1 rounded-full hover:bg-teal-100 transition-colors">
                  ...
                  return (
                    <button key={variant.id} onClick={() => !isOutOfStock && setSelectedVariant(variant)}
                      ...
                      className={`px-3.5 py-1.5 rounded-full text-sm font-semibold border transition-all duration-150
                        ${isSelected ? 'bg-teal-400 border-teal-400 text-white shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-teal-400 hover:text-teal-600'}
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <button onClick={handleAddToCart} disabled={!selectedVariant || outOfStock}
                className={`flex-1 py-3.5 rounded-full font-black text-sm transition-all duration-200
                  ${cartFeedback ? 'bg-green-500 text-white scale-95' : 'bg-teal-400 hover:bg-teal-500 active:bg-teal-600 text-white'}
        ...
        {product.seller && (
          <div className="mt-10 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href={`/shops/${product.seller.id}`} className="text-sm font-bold text-teal-600 hover:underline">
                View Shop
              </Link>
            </div>
              ...
              {reviewSuccess ? (
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4">
                  ...
                  <button type="submit" disabled={reviewLoading}
                    className="bg-teal-400 hover:bg-teal-500 disabled:opacity-60 text-white font-black px-6 py-2.5 rounded-xl text-sm transition-colors">
              ...
              <div className="space-y-4">
                {reviews.map(review => (
                  <div key={review.id} className="flex gap-3 border-b border-gray-50 last:border-0 pb-4 last:pb-0">
                    <div className="w-8 h-8 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center font-black text-xs shrink-0">
