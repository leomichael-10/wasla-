...
- ~~**Product listing zone-awareness**~~ — **closed**: `/browse`, `/products`, and the homepage now grey out (not hide) products from shops outside the visitor's selected zone and disable Add to Cart, in addition to the cart's existing checkout-time enforcement.
- **Real-time order notifications** — the dashboard order queue polls every 20 seconds rather than using WebSockets/SSE.
- **Zone gate robustness** — fixed a real bug post-launch: the modal could get stuck with no zone options and no escape if the `/api/zones` fetch failed, since errors were silently swallowed and there was no close button. Now has an explicit loading/zones/error/waitlist-thanks view state, a retry-on-error path, and an always-available close button.
- **PWA install** — added: manifest, hand-rolled service worker (not `next-pwa`/`serwist` — Next 16 compatibility unconfirmed, see DECISIONS.md), Android/Chrome install-prompt capture, iOS Share-sheet instruction fallback. Verified manifest/service-worker/icons serve correctly from a production build; **not** verified on an actual deployed preview URL (no deployment access in this environment).
