 ...
 * item shape:
 * {
 *   productVariantId, productId, productName, productNameEn, image, brand,
 *   label, price,
 *   quantity, sellerId, sellerName
 * }
 * productNameEn may be '' — display code should fall back to productName,
 * same rule as lib/i18n.js's productName() for a live Product row.
 * image may be null/undefined (no photo, or an item added before this
 * field existed — components/MediaThumb.js already renders that as its
 * quiet fallback mark, no special-casing needed at the call site).
 */
...
export function getCartTotal(userId) {
  return getCart(userId).reduce((sum, c) => sum + c.price * c.quantity, 0)
}
