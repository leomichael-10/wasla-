...
function shell(content) {
  ...
  <div class="wrap">
    ...
    ${content}
    <div class="footer">Tobaki &mdash; UAE&rsquo;s First Vape Marketplace &bull; <a href="https://tobaki.ae/terms" style="color:#9ca3af;">Terms</a> &bull; <a href="https://tobaki.ae/privacy" style="color:#9ca3af;">Privacy</a></div>
  </div>
  </body></html>`
}
...
function itemsTable(items) {
  return `
    <table class="items">
      <thead><tr>
        <th>Product</th><th>Flavor</th><th>Qty</th><th>Price</th>
      </tr></thead>
      <tbody>
        ${items.map(it => {
          ...
          return `<tr>
            ...
            <td>${it.quantity}</td>
            <td>AED ${Number(it.priceAtPurchase).toFixed(2)}</td>
          </tr>`
        }).join('')}
      </tbody>
    </table>
...
export function orderConfirmation(order, items) {
  return shell(`
    <div class="body">
      ...
      <p>Thank you for your order on Tobaki. Here&rsquo;s your summary:</p>
      <p><span class="badge">Order #${order.id}</span></p>
      ${itemsTable(items)}
      <p>Delivery address: <strong>${order.deliveryAddress}</strong></p>
      <p class="total">Total: AED ${Number(order.totalAed).toFixed(2)}</p>
      <p>Your seller will confirm the order shortly. You&rsquo;ll receive a notification when it&rsquo;s accepted.</p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://tobaki.ae'}/orders" class="btn">Track My Order</a>
    </div>
  `)
}
...
export function orderAccepted(order) {
  return shell(`
    <div class="body">
      ...
      <p>Great news! Your Tobaki order <span class="badge">#${order.id}</span> has been accepted and is being prepared.</p>
      <p>Estimated delivery: <strong>2–4 hours</strong></p>
      <p>Delivery address: <strong>${order.deliveryAddress}</strong></p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://tobaki.ae'}/orders" class="btn">View Order</a>
    </div>
  `)
}
...
export function orderDelivered(order) {
  return shell(`
    <div class="body">
      ...
      <p>Your Tobaki order <span class="badge">#${order.id}</span> has been delivered. We hope you enjoy your products!</p>
      <p>Thank you for shopping with Tobaki.</p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://tobaki.ae'}/products" class="btn">Shop Again</a>
    </div>
  `)
}
...
export function newOrderAlert(order, items) {
  return shell(`
    <div class="body">
      ...
      <p>You have a new order on Tobaki:</p>
      <p><span class="badge">Order #${order.id}</span></p>
      ${itemsTable(items)}
      <p>Delivery address: <strong>${order.deliveryAddress}</strong></p>
      <p class="total">Total: AED ${Number(order.totalAed).toFixed(2)}</p>
      <p>Please log in to your dashboard to accept or cancel the order.</p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://tobaki.ae'}/dashboard/orders" class="btn">Go to Dashboard</a>
    </div>
  `)
}
...
export function subscriptionConfirmation(sellerName, plan) {
  return shell(`
    <div class="body">
      ...
      <p>Hi <strong>${sellerName}</strong>, we&rsquo;ve received your subscription request for Tobaki.</p>
      <p><strong>Plan:</strong> AED ${plan} / month</p>
...
export function subscriptionActivated(sellerName) {
  return shell(`
    <div class="body">
      ...
      <p>Hi <strong>${sellerName}</strong>, your Tobaki seller subscription has been activated. Your shop is now live on the platform.</p>
      <p>Start adding products and reach thousands of customers across the UAE.</p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://tobaki.ae'}/dashboard" class="btn">Go to Dashboard</a>
    </div>
  `)
}
