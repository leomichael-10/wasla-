...
// Must always be a publicly-reachable https URL — every email built here
// embeds it as an absolute asset/link base, and a mail client can never
// resolve localhost or a stale/unowned domain. Set NEXT_PUBLIC_APP_URL in
// Vercel's project settings for production (https://www.wasla-249.com);
// this fallback only exists as a last resort if that's ever unset.
export const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.wasla-249.com'
...
const BASE = `
  <style>
    ...
    .code { font-size: 28px; font-weight: 900; letter-spacing: 6px; color: #6F4E37; margin: 12px 0; }
...
function shell(content, dir = 'ltr') {
  ...
  <div class="wrap">
    <div class="header">
      <img src="${APP_URL}/icon-192.png" width="56" height="56" alt="Wasla" style="width:56px;height:56px;border:0;outline:none;text-decoration:none;" />
      <h1><span>W</span>asla</h1>
    </div>
    ${content}
    ...
    <table class="items">
      ...
      <tbody>
        ${items.map(it => {
          ...
          return `<tr>
            <td>${product?.nameEn?.trim() || product?.name || 'Product'}</td>
            <td>${variant?.label ?? '—'}</td>
            <td>${it.quantity}</td>
...
export function orderConfirmation(order, items) {
  ...
  return shell(`
    <div class="body">
      ...
      <a href="${APP_URL}/orders" class="btn">تتبع طلبي</a>
    </div>
  `, 'rtl')
}
...
export function orderDelivered(order) {
  return shell(`
    <div class="body">
      ...
      <a href="${APP_URL}/products" class="btn">Shop Again</a>
    </div>
  `)
}
...
export function newOrderAlert(order, items) {
  return shell(`
    <div class="body">
      ...
      <a href="${APP_URL}/dashboard/orders" class="btn">Go to Dashboard</a>
    </div>
  `)
}
...
// Password reset — sent by lib/verification/passwordReset.js. The code is
// the same secret whether the person clicks the link or types it in by
// hand (see that file's comment for why one value doing both jobs is
// deliberate, not a shortcut).
export function passwordResetEmail({ code, resetUrl }) {
  const subject = t('email.passwordReset.subject', 'ar')
  // resetUrl's query string has a literal `&` separating email/code — raw
  // inside an HTML attribute that's technically invalid markup, and some
  // mail-client HTML sanitizers (notably Outlook desktop's Word renderer)
  // are stricter than a browser about ambiguous ampersands. Escape it once
  // here rather than relying on every caller to remember to.
  const hrefUrl = resetUrl.replace(/&/g, '&amp;')
  const html = shell(`
    <div class="body">
      <h2>${t('email.passwordReset.heading', 'ar')}</h2>
      <p>${t('email.passwordReset.body1', 'ar')}</p>
      <a href="${hrefUrl}" class="btn">${t('email.passwordReset.cta', 'ar')}</a>
      <p style="margin-top:24px">${t('email.passwordReset.codeLabel', 'ar')}</p>
      <p class="code">${code}</p>
      <p style="color:#9ca3af;font-size:12px">${t('email.passwordReset.expiry', 'ar')}</p>
      <hr class="divider" />
      <div dir="ltr" style="text-align:left">
        <h2>${t('email.passwordReset.heading', 'en')}</h2>
        <p>${t('email.passwordReset.body1', 'en')}</p>
        <a href="${hrefUrl}" class="btn">${t('email.passwordReset.cta', 'en')}</a>
        <p style="margin-top:24px">${t('email.passwordReset.codeLabel', 'en')}</p>
        <p class="code">${code}</p>
        <p style="color:#9ca3af;font-size:12px">${t('email.passwordReset.expiry', 'en')}</p>
      </div>
    </div>
  `, 'rtl')
  const text = plainText([
    t('email.passwordReset.heading', 'ar'),
    t('email.passwordReset.body1', 'ar'),
    `${t('email.passwordReset.cta', 'ar')}: ${resetUrl}`,
    `${t('email.passwordReset.codeLabel', 'ar')} ${code}`,
    t('email.passwordReset.expiry', 'ar'),
    '---',
    t('email.passwordReset.heading', 'en'),
    t('email.passwordReset.body1', 'en'),
    `${t('email.passwordReset.cta', 'en')}: ${resetUrl}`,
    `${t('email.passwordReset.codeLabel', 'en')} ${code}`,
    t('email.passwordReset.expiry', 'en'),
  ])
  return { subject, html, text }
}

// Sent instead of a reset code when someone requests a reset for an
// email that's registered but Google-only (no passwordHash) — see
// lib/verification/passwordReset.js. Safe to be this specific here since
// only the account owner (whoever controls that inbox) ever sees it; the
// HTTP response to the reset *request* stays identical either way.
export function googleAccountResetEmail() {
  const subject = t('email.googleAccountReset.subject', 'ar')
  const loginUrl = `${APP_URL}/login`
  const html = shell(`
    <div class="body">
      <h2>${t('email.googleAccountReset.heading', 'ar')}</h2>
      <p>${t('email.googleAccountReset.body1', 'ar')}</p>
      <p>${t('email.googleAccountReset.body2', 'ar')}</p>
      <a href="${loginUrl}" class="btn">${t('email.googleAccountReset.cta', 'ar')}</a>
      <hr class="divider" />
      <div dir="ltr" style="text-align:left">
        <h2>${t('email.googleAccountReset.heading', 'en')}</h2>
        <p>${t('email.googleAccountReset.body1', 'en')}</p>
        <p>${t('email.googleAccountReset.body2', 'en')}</p>
        <a href="${loginUrl}" class="btn">${t('email.googleAccountReset.cta', 'en')}</a>
      </div>
    </div>
  `, 'rtl')
  const text = plainText([
    t('email.googleAccountReset.heading', 'ar'),
    t('email.googleAccountReset.body1', 'ar'),
    t('email.googleAccountReset.body2', 'ar'),
    `${t('email.googleAccountReset.cta', 'ar')}: ${loginUrl}`,
    '---',
    t('email.googleAccountReset.heading', 'en'),
    t('email.googleAccountReset.body1', 'en'),
    t('email.googleAccountReset.body2', 'en'),
    `${t('email.googleAccountReset.cta', 'en')}: ${loginUrl}`,
  ])
  return { subject, html, text }
}
