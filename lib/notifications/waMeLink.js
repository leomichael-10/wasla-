...
/** Strip everything but digits, then drop a leading 0 (wa.me wants country code + local number, no leading 0). */
...
/** Pure — the wa.me deep link itself. Returns null if the shop has no number on file. */
export function buildWaMeUrl(order, shop) {
  const phone = normalizePhone(shop?.whatsappNumber)
